...
[type=verification]
Have you been able to access the interface from Swagger UI? Do you see the updated documentation?
...
Now we have the REST endpoints but it doesn't do anything until we tie it with some kind of back-end data store. Let's work on interaction with the orders database to retrieve relevant order information. For this we will use the link:https://camel.apache.org/sql.html[camel-SQL, window="_blank"] component to do it.
