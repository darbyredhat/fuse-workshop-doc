...
As you can see the queue we need is not created. Follow theses steps to do so:
...
[source,bash,subs="attributes+"]
----
while(true); do curl http://fuse-workshop-{user-username}-fuse.{openshift-host}/camel/orders; sleep 1; done
----
