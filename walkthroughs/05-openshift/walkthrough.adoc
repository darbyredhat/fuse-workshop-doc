...
Then look for *mysql* & select the *MySQL (Ephemeral)* option
...
Fill *MySQL Connection Username, MySQL Connection Password, & MySQL root user Password* as *redhat* and then click *Create*.
...
Replace `{user-username}` with your user name, then click *Create*.
...
. On the build and deploy are done, you will be able to see the container and in the following image. You may try it by click in the Route button.
+
